...
| Doc | Title | Tags |
|---|---|---|
| Wiki | `📚 <Library> — Wiki` | `llm-wiki`, `<library name>`, `llm-wiki-page` |
| Index | `🗂 <Library> — Wiki Index` | `llm-wiki`, `<library name>`, `llm-wiki-index` |
| Log | `🪵 <Library> — Wiki Log` | `llm-wiki`, `<library name>`, `llm-wiki-log` |

All three are private (never added to the library). `<library name>` is the library's exact display name (also usable as a tag to scope discovery to one library).

**Discovery filter:** TK's tag filter is **OR**, so query by the specific `<library name>` tag, then keep only results that ALSO carry `llm-wiki` (both tags must be present — a bare `llm-wiki` match belongs to some other library). The role tag (`llm-wiki-page|index|log`) then says which of the three each is.
...
## Skipped (non-source)
- `<id>` <title> — <reason it carries no wiki content, e.g. empty / test noise / duplicate>
...
`<tk-url>` is the source's real URL (`http://localhost:5173/notes/<id>` locally, the prod URL for `tk-remote`) — it carries the node id used for delta detection. **`ingested @ <updated_at>`** is the source's `updated_at` at ingest time; the next ingest compares it against the live value to detect edits. **`## Skipped (non-source)`** records docs deliberately examined and excluded so they aren't re-read (a transient read *failure* is instead left unrecorded, so it retries automatically next run).
...
| Enumerate sources (+ deltas) | `get_library_documents` (`since`, `sort_by`, `limit`, `offset`, `has_more`) |
